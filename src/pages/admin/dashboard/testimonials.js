  ...
  const columns = [
    {
      ...
      headerName: "Name",
      flex: 1,
      valueGetter: (params) => params.row.testimonialName,
    },
    {
      field: "heading",
      headerName: "Review Heading",
      flex: 1,
      valueGetter: (params) => params.row.testimonialHeading,
    },
    {
      field: "stars",
      headerName: "Stars",
      flex: 1,
      valueGetter: (params) => params.row.stars,
    },
    {
      field: "options",
      headerName: "Options",
      ...
      renderCell: (params) => {
        const onClickEdit = () => {
          ...
          setAddNewForm(true);
          setSelectedTestimonialForUpdate(params.row);
        };

        const onClickDelete = () => {
          if (
            window.confirm(
              `Are you sure you want to delete the review by ${params.row.testimonialName}?`
            )
          ) {
            deleteTestimonial(params.row.id);
          }
        };
        return (
          <>
