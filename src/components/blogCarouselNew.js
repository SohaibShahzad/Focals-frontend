...
export const CustomCarousel = ({ items, itemsToShow = 3 }) => {
  ...
  return (
    <div ref={carouselRef} className="relative flex flex-col items-center gap-10">
      <div className="gradient-03" />
      <div className={`gap-5 flex transition-all duration-500 ease-in-out`}>
        {items.length === 0 ? (
          <div className={`p-5 glassmorphism rounded-lg h-[300px] w-[250px] mx-2`}>
          ...
          visibleItems.map((item, index) => (
            <motion.div
              ...
              animate="center"
              exit="exit"
              className="p-5 glassmorphism hover:bg-orange-800 rounded-lg mx-2 flex flex-col justify-between h-[300px] w-[250px]"
              transition={{
                x: { type: "spring", stiffness: 300, damping: 30 },
                opacity: { duration: 0.2 },
              }}
            >
              ...
              key={item._id}
              href={`/blogs/${item._id}`}
              className="grid grid-rows-[auto,1fr,auto] gap-5 h-full justify-between">
                <div className="flex justify-center items-center h-[150px] overflow-hidden">
                  <img
                    src={item.image}
                    alt={item.title}
                    className="rounded-md w-full h-full object-cover transform transition-all duration-300 hover:scale-110"
                  />
                </div>
                <div>
                  <h3 className="text-center font-bold text-[24px] ">
